...
export const Nav = ({ connectWalletButton }: Props) => {
  return (
    <nav>
      ...
      <DisplayWrapper>
        <div className="fixed left-0 top-0 z-50 flex h-full w-full shrink-0 flex-col bg-gradient-to-l from-[#8743FF] to-[#4136F1] p-4 py-2 md:hidden">
          <NavContent isMobile connectWalletButton={connectWalletButton} />
        </div>
      </DisplayWrapper>

      {/* Desktop Sidebar Nav */}
      <div className="fixed left-0 top-0 hidden h-full w-full max-w-fit shrink-0 flex-col border-r-2 border-white/5 p-4 md:flex">
